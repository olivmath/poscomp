...
exports.finishSimulado = (0, https_1.onCall)(async (request) => {
    ...
    const { simuladoId, answers, timeSpentSeconds } = request.data;
    ...
    if (!simuladoId || typeof simuladoId !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'simuladoId is required');
    }
    ...
    if (!Array.isArray(answers) || answers.length === 0) {
        throw new https_1.HttpsError('invalid-argument', 'answers cannot be empty');
    }
    ...
    for (const a of answers) {
        if (!Number.isInteger(a.questionId) || a.questionId <= 0) {
            throw new https_1.HttpsError('invalid-argument', `Invalid questionId: ${a.questionId}`);
        }
        if (!types_1.VALID_OPTIONS.includes(a.selected)) {
            throw new https_1.HttpsError('invalid-argument', `Invalid selected option: ${a.selected}`);
        }
        if (!types_1.VALID_CONFIDENCES.includes(a.confidence)) {
            throw new https_1.HttpsError('invalid-argument', `Invalid confidence: ${a.confidence}`);
        }
    }
    ...
    if (missingIds.length > 0) {
        const missingChunks = (0, chunks_1.chunkArray)(missingIds, 30);
    ...
    const answersOutput = answers.map((a) => {
        ...
        if (correct)
            score++;
        const m = q.materia;
        if (!materiaBreakdown[m])
            materiaBreakdown[m] = { correct: 0, total: 0 };
        materiaBreakdown[m].total++;
        if (correct)
            materiaBreakdown[m].correct++;
        return {
